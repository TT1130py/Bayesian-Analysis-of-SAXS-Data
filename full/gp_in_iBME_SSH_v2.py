...
os.chdir('/home/tjaglal/iBME')
print("Current working directory: {0}".format(os.getcwd()))

#Parameters for do_gp
path_structures = "/home/tjaglal/Structures/all_structures/mm"
path_exp_file = "/home/tjaglal/Structures/Experiment"
theta = "50" #theta
...
dro_grid = np.arange(0.0, 50.0 + 5.0, 5.0) 
r0_grid =  np.arange(1.0, 1.5 + 0.2, 0.2)
...
run = subprocess.run(["./do_gp_v3_SSH.sh", path_structures, "{}/SASDLU4.dat".format(path_exp_file), theta, gl, os.path.join(sub_path, "grid_run.txt")], stdout=subprocess.PIPE, stderr=subprocess.PIPE, env=env)
...
for i in range(len(GRID_DF)):
    ...
    try:
        ...
        run2 = iBME_mod_SSH.iBMEf("{}/SASDLU4_trun.dat".format(path_exp_file), 
...
for i in range(len(GRID_DF)):
    ...
    if not os.path.exists(folder_path):
        ...
        continue
    frames = pd.read_csv(folder_path, delim_whitespace=True)
...
shutil.copytree(f'{sub_path}/GP{weight_str}', copy_dir)

contents.to_csv(f'{info_path}/content_id_{today}_{dir_num}.txt')
